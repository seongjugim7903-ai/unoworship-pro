#!/bin/bash
# =============================================================
# scripts/mac-output-kiosk.sh
# MAIN / 강대상 모니터(FullHD TV)에 /main 키오스크 전체화면 자동 실행
#
# 핵심 원칙:
#   - 기존 Chrome 세션을 절대 건드리지 않음
#   - 별도 프로필(--user-data-dir)로 키오스크 전용 Chrome 인스턴스 실행
#   - 이전 키오스크 인스턴스만 선택적으로 종료
#
# 사용법:
#   ./scripts/mac-output-kiosk.sh          # 일반 실행 (서버 대기 후 즉시 실행)
#   ./scripts/mac-output-kiosk.sh --boot   # 부팅 시 자동 실행 (디스플레이 안정화 대기 포함)
#
# ── 설정 ────────────────────────────────────────────────────
...
PORT=$SERVER_PORT
OUTPUT_URL="${SERVER_URL}/main"
KIOSK_PROFILE="$PROFILE_OUTPUT"
MONITOR_X=$MONITOR_OUTPUT_X
MONITOR_Y=$MONITOR_OUTPUT_Y
MONITOR_W=$MONITOR_OUTPUT_W
MONITOR_H=$MONITOR_OUTPUT_H
# =============================================================

LOG_FILE="$HOME/Library/Logs/unolive-kiosk.log"
...
# ── 이전 키오스크 인스턴스만 선택적 종료 ─────────────────────
# 공유 런타임 프로필에서는 이 프로필에 제어/프롬프트/카메라 창도 함께 있으므로
...
if [ "${UNOLIVE_SHARED_RUNTIME_PROFILE:-0}" != "1" ]; then
  ...
  if [ -n "$KIOSK_PIDS" ]; then
    log "Terminating previous kiosk instance (PIDs: $KIOSK_PIDS)"
...
# ── Chrome 키오스크 모드 실행 ─────────────────────────────────
# open -na : 기존 Chrome과 별개의 새 인스턴스로 실행
log "Launching Chrome kiosk on monitor X=${MONITOR_X}..."
