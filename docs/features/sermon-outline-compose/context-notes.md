...
주보 이미지를 올리면 **예배 순서 세 가지만** 뽑는다 — 주일낮 · 주일오후 · 수요. 사용자가 범위를 그렇게 정했다. 교회소식과 금요기도회는 뽑지 않는다(교회소식은 자체 탭에서 직접 입력받는다).

**기존 추출기를 쓰지 않고 새로 만든 이유.** `lib/bulletin/extractBulletin.ts` 는 다섯 섹션(교회소식·금요기도회 포함)을 뽑는 프롬프트다. 세 가지만 필요한데 다섯을 요구하면 프롬프트가 길어져 정확도가 떨어지고 토큰도 낭비된다. 수정 금지 대상이기도 해서 `lib/sermon-compose/bulletinExtract.ts` 에 세 가지 전용 프롬프트를 따로 뒀다. 모델은 기존과 같은 `claude-opus-4-8` — 실사용으로 검증된 조합을 그대로 쓴다.

**면마다 따로 읽고 합친다.** 주보는 앞뒤 여러 면이라 한 예배 순서가 두 면에 걸칠 수 있다. 면별로 호출한 뒤 같은 예배끼리 줄바꿈으로 이어 붙인다 — 내용을 버리지 않는다.

**업로드 즉시 자동 분석 (사용자 요구).** 파일을 고르면 압축 후 바로 분석까지 이어 간다. 버튼을 누를 필요가 없다.

- 자동(`append`) — 새로 올린 면만 읽어 기존 결과 뒤에 잇는다. 나중에 한 면을 더 올려도 앞서 읽은 것과 고친 내용이 살아 있다
- 수동(`replace`) — `전체 다시 분석` 버튼. 올린 면 전체를 다시 읽어 결과를 새로 만든다. 편집한 내용이 사라지므로 그 사실을 버튼 아래에 적어 뒀다

두 동작이 다르므로 버튼 이름을 `전체 다시 분석` 으로 명확히 했다. 분석 실패 후 재시도 경로가 필요해서 버튼을 없애지 않았다.

**최신 순서를 ref 로도 들고 간다.** `append` 병합에는 직전 결과가 필요한데, `setState` 업데이터 안에서 병합 결과를 밖으로 빼내는 편법은 StrictMode 에서 두 번 실행될 수 있다. `ordersRef` 를 두고 `applyOrders()` 한 곳에서만 상태와 함께 갱신한다.

실제 Claude 호출로 검증했다. 예배 순서표를 그린 이미지 한 장으로 세 예배가 모두 `항목: 내용` 형태로 뽑혔다(주일낮 8줄·주일오후 4줄·수요 4줄).

### 교회소식 탭 (2026-08-02)

`kind='news'` 를 추가했다. 빈 줄(엔터 두 번)마다 소식 한 건이 한 섹션이 된다.

분할 규칙은 UnoLive `worshipServiceGenerator.ts` 의 교회소식 빌더와 **글자 그대로 같아야 한다.**

```
newsText.split(/\n\s*\n+/).map(trim).filter(Boolean)
```

규칙이 어긋나면 입력 화면 미리보기에서 본 장수와 현장에서 실제로 만들어지는 장수가 달라진다. 그래서 `lib/sermon-compose/churchNews.ts` 한 곳에만 두고 고정 테스트로 묶었다 — 빈 줄 연속, 공백만 있는 줄, 앞뒤 빈 줄, 한 건 안의 줄바꿈 유지까지.

서버에서도 원문(`content`)을 받아 **같은 함수로 다시 나눈다.** 클라이언트가 보낸 블록 배열을 그대로 믿지 않는다.
