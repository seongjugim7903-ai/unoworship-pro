...
`placement` 는 항목마다 둘 중 하나다.

| 값 | 동작 |
|---|---|
| `quote-tail` (기본) | `말씀찾기(인용)` 프로그램 **맨 뒤에 섹션으로 추가** |
| `own-program` | `{YYYYMMDD}-설교참고자료-{n}` 별도 프로그램으로 생성 |

기본값을 `quote-tail` 로 둔 이유는 프로그램 개수가 늘지 않아 예배 중 방송실이 찾을 것이 적기 때문이다. 길거나 독립적으로 띄워야 하는 자료만 `own-program` 으로 바꾼다.
